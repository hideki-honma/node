...
Interface.prototype.setBreakpoint = function(script, line,
  ...
  Record.setBreakpoint(self, script, line, condition, silent);
};
...
Interface.prototype.createRecord = function(recordName, options) {
  this.record = new Record(recordName, 'w');
  this.record.createRecord();
};

Interface.prototype.readRecord = function(recordName, options) {
  this.record = new Record(recordName, 'r');
  this.record.readRecord(this);
};

function Record(recordName, mode) {
  var instance = this;
  this.recordfile=null;
  this.mode=null;
  var propset = function(recordName, mode) {
    if(recordName == undefined || recordName == null) {
      instance.recordfile = "debug.record";
    } else {
      instance.recordfile = recordName;
    }
    instance.mode = mode;
  };
  propset(recordName, mode);

  Record = function(recordName, mode) {
    propset(recordName, mode);
    return instance;
  };

  Record.setBreakpoint = function(_interface, script, line, condition, silent) {
    if(_interface.record !== undefined && _interface.record['mode'] === 'w') {
      _interface.record.doSetBreakpoint(script, line, condition, silent);
    }
  };

  Record.setWatch = function(_interface, expr) {
    if(_interface.record !== undefined && _interface.record['mode'] === 'w') {
      _interface.record.doSetWatch(expr);
    }
  };
};

Record.prototype.createRecord = function() {
  console.log("record start!");
  if(!this.writeStream) {
    this.writeStream = fs.createWriteStream(this.recordfile, {
      flags: 'w',
  ...
  this.writeStream.on('drain', function ()          { console.log('write: drain'); })
                  .on('error', function (exception) { console.log('write: error'); console.log(exception); })
                  .on('close', function ()          { console.log('write: close'); })
                  .on('pipe',  function (src)       { console.log('write: pipe');  })
                  ;
};

Record.prototype.readRecord = function(_interface) {
  this.readStream = fs.createReadStream(this.recordfile, {
    encoding: 'utf-8',
    buffersize: 100,
    autoClose: true
  });
  var isfirst = true;
  var rl = readline.createInterface(this.readStream, {});
    rl.on('line', function(line) {
    var recordline = JSON.parse(line);
    if(recordline["breakpoint"]) {
      var breakpoint = recordline.breakpoint;
      if(!isfirst) breakpoint.silent = 1;
      else isfirst = false;
      _interface.setBreakpoint(breakpoint.script, Number(breakpoint.line), breakpoint.condition, breakpoint.silent);
    } else if(recordline["watch"]) {
      _interface.watch(recordline.watch);
    }
  });
};

Record.prototype.doSetBreakpoint = function(script, line, condition, silent) {
  if(this.mode != 'w') {
    return;
  }
  if(!script) script="";
  ...
  if(!silent) silent="";
  this.writeStream.write('{ "breakpoint" : {');
  this.writeStream.write('"script":"'+script+'",');
  this.writeStream.write('"line":"'+line+'",');
  this.writeStream.write('"condition":"'+condition+'",');
  this.writeStream.write('"silent":"'+silent+'"');
  this.writeStream.write('}}\n');
};

Record.prototype.doSetWatch = function(expr) {
  this.writeStream.write('{ "watch" : "' + expr + '" }\n');
};
